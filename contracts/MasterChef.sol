...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to LqdrSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // LqdrSwap must mint EXACTLY the same amount of LqdrSwap LP tokens or
...
contract MasterChef is Ownable {
    ...
    // Block number when bonus LQDR period ends.
    ...
    // Update the given pool's LQDR allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 lqdrReward =
                multiplier.mul(lqdrPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accLqdrPerShare = accLqdrPerShare.add(
                lqdrReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accLqdrPerShare = pool.accLqdrPerShare.add(
            lqdrReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accLqdrPerShare).div(1e12).sub(
                    user.rewardDebt
                );
